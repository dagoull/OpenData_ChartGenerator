...
public class GenerarSalidaGraficoPastel implements IGenerarSalida
{
	...
	private void configurarDataSet()
	{
		...
		for (int i = 0; i < t_barrios.size(); i++)
		{
			...
			for (int j = 0; j < t_arrInstancias.size(); j++)
			{
				...
				Double t_totalTuristasPorBarrio = Double.parseDouble(t_instancia.getValorItem(26))+Double.parseDouble(t_instancia.getValorItem(48));
	...
	public void salida()
	{
		m_salida = ChartFactory.createPieChart("Grafico de Pastel", m_datasetChartPie, true, true, false);
		
		PiePlot plot = (PiePlot) m_salida.getPlot();//añade el porcetaje a la etiqueta
		plot.setSimpleLabels(true);
		PieSectionLabelGenerator gen = new StandardPieSectionLabelGenerator(
	            "{0}: {1} ({2})", new DecimalFormat("0"), new DecimalFormat("0%"));//Formatea el porcentaje
